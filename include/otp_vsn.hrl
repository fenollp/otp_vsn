%% Copyright © 2018 Pierre Fenoll ‹pierrefenoll@gmail.com›
%% See LICENSE for licensing information.
%% -*- coding: utf-8 -*-
%% Auto-generated file. See https://github.com/fenollp/otp_vsn
-ifndef(_OTP_VSN_HRL).
-define(_OTP_VSN_HRL, true).
-include_lib("otp_vsn/include/internal_otp_vsn.hrl").

-ifdef(_internal_otp_vsn_16_0_0).
-define(OTP_VSN, "16.0.0").
-define(OTP_VSN_MAJOR, 16).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_16_1_0).
-define(OTP_VSN, "16.1.0").
-define(OTP_VSN_MAJOR, 16).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_16_2_0).
-define(OTP_VSN, "16.2.0").
-define(OTP_VSN_MAJOR, 16).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_16_3_0).
-define(OTP_VSN, "16.3.0").
-define(OTP_VSN_MAJOR, 16).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_16_3_1).
-define(OTP_VSN, "16.3.1").
-define(OTP_VSN_MAJOR, 16).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_0_0).
-define(OTP_VSN, "17.0.0").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_0_1).
-define(OTP_VSN, "17.0.1").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_0_2).
-define(OTP_VSN, "17.0.2").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_1_0).
-define(OTP_VSN, "17.1.0").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_1_1).
-define(OTP_VSN, "17.1.1").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_1_2).
-define(OTP_VSN, "17.1.2").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_2_0).
-define(OTP_VSN, "17.2.0").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_2_1).
-define(OTP_VSN, "17.2.1").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_2_2).
-define(OTP_VSN, "17.2.2").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_3_0).
-define(OTP_VSN, "17.3.0").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_3_1).
-define(OTP_VSN, "17.3.1").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_3_2).
-define(OTP_VSN, "17.3.2").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_3_3).
-define(OTP_VSN, "17.3.3").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_3_4).
-define(OTP_VSN, "17.3.4").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_4_0).
-define(OTP_VSN, "17.4.0").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 4).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_4_1).
-define(OTP_VSN, "17.4.1").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 4).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_5_0).
-define(OTP_VSN, "17.5.0").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 5).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_5_1).
-define(OTP_VSN, "17.5.1").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 5).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_5_2).
-define(OTP_VSN, "17.5.2").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 5).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_5_3).
-define(OTP_VSN, "17.5.3").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 5).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_5_4).
-define(OTP_VSN, "17.5.4").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 5).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_5_5).
-define(OTP_VSN, "17.5.5").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 5).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_17_5_6).
-define(OTP_VSN, "17.5.6").
-define(OTP_VSN_MAJOR, 17).
-define(OTP_VSN_MINOR, 5).
-define(OTP_VSN_PATCH, 6).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_0_0).
-define(OTP_VSN, "18.0.0").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_0_1).
-define(OTP_VSN, "18.0.1").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_0_2).
-define(OTP_VSN, "18.0.2").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_0_3).
-define(OTP_VSN, "18.0.3").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_1_0).
-define(OTP_VSN, "18.1.0").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_1_1).
-define(OTP_VSN, "18.1.1").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_1_2).
-define(OTP_VSN, "18.1.2").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_1_3).
-define(OTP_VSN, "18.1.3").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_1_4).
-define(OTP_VSN, "18.1.4").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_1_5).
-define(OTP_VSN, "18.1.5").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_2_0).
-define(OTP_VSN, "18.2.0").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_2_1).
-define(OTP_VSN, "18.2.1").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_2_2).
-define(OTP_VSN, "18.2.2").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_2_3).
-define(OTP_VSN, "18.2.3").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_2_4).
-define(OTP_VSN, "18.2.4").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_3_0).
-define(OTP_VSN, "18.3.0").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_3_1).
-define(OTP_VSN, "18.3.1").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_3_2).
-define(OTP_VSN, "18.3.2").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_3_3).
-define(OTP_VSN, "18.3.3").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_18_3_4).
-define(OTP_VSN, "18.3.4").
-define(OTP_VSN_MAJOR, 18).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_0_0).
-define(OTP_VSN, "19.0.0").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_0_1).
-define(OTP_VSN, "19.0.1").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_0_2).
-define(OTP_VSN, "19.0.2").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_0_3).
-define(OTP_VSN, "19.0.3").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_0_4).
-define(OTP_VSN, "19.0.4").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_0_5).
-define(OTP_VSN, "19.0.5").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_0_6).
-define(OTP_VSN, "19.0.6").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 6).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_0_7).
-define(OTP_VSN, "19.0.7").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 7).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_1_0).
-define(OTP_VSN, "19.1.0").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_1_1).
-define(OTP_VSN, "19.1.1").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_1_2).
-define(OTP_VSN, "19.1.2").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_1_3).
-define(OTP_VSN, "19.1.3").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_1_4).
-define(OTP_VSN, "19.1.4").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_1_5).
-define(OTP_VSN, "19.1.5").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_1_6).
-define(OTP_VSN, "19.1.6").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 6).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_2_0).
-define(OTP_VSN, "19.2.0").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_2_1).
-define(OTP_VSN, "19.2.1").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_2_2).
-define(OTP_VSN, "19.2.2").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_2_3).
-define(OTP_VSN, "19.2.3").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_3_0).
-define(OTP_VSN, "19.3.0").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_3_1).
-define(OTP_VSN, "19.3.1").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_3_2).
-define(OTP_VSN, "19.3.2").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_3_3).
-define(OTP_VSN, "19.3.3").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_3_4).
-define(OTP_VSN, "19.3.4").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_3_5).
-define(OTP_VSN, "19.3.5").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_19_3_6).
-define(OTP_VSN, "19.3.6").
-define(OTP_VSN_MAJOR, 19).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 6).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_0_0).
-define(OTP_VSN, "20.0.0").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_0_1).
-define(OTP_VSN, "20.0.1").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_0_2).
-define(OTP_VSN, "20.0.2").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_0_3).
-define(OTP_VSN, "20.0.3").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_0_4).
-define(OTP_VSN, "20.0.4").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_0_5).
-define(OTP_VSN, "20.0.5").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_1_0).
-define(OTP_VSN, "20.1.0").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_1_1).
-define(OTP_VSN, "20.1.1").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_1_2).
-define(OTP_VSN, "20.1.2").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_1_3).
-define(OTP_VSN, "20.1.3").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_1_4).
-define(OTP_VSN, "20.1.4").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_1_5).
-define(OTP_VSN, "20.1.5").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_1_6).
-define(OTP_VSN, "20.1.6").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 6).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_1_7).
-define(OTP_VSN, "20.1.7").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 7).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_2_0).
-define(OTP_VSN, "20.2.0").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_2_1).
-define(OTP_VSN, "20.2.1").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_2_2).
-define(OTP_VSN, "20.2.2").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_2_3).
-define(OTP_VSN, "20.2.3").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_2_4).
-define(OTP_VSN, "20.2.4").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_3_0).
-define(OTP_VSN, "20.3.0").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_3_1).
-define(OTP_VSN, "20.3.1").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_3_2).
-define(OTP_VSN, "20.3.2").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_3_3).
-define(OTP_VSN, "20.3.3").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_3_4).
-define(OTP_VSN, "20.3.4").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_3_5).
-define(OTP_VSN, "20.3.5").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_3_6).
-define(OTP_VSN, "20.3.6").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 6).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_3_7).
-define(OTP_VSN, "20.3.7").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 7).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_20_3_8).
-define(OTP_VSN, "20.3.8").
-define(OTP_VSN_MAJOR, 20).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 8).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_0_0).
-define(OTP_VSN, "21.0.0").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_0_1).
-define(OTP_VSN, "21.0.1").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_0_2).
-define(OTP_VSN, "21.0.2").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_0_3).
-define(OTP_VSN, "21.0.3").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_0_4).
-define(OTP_VSN, "21.0.4").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_0_5).
-define(OTP_VSN, "21.0.5").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_0_6).
-define(OTP_VSN, "21.0.6").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 6).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_0_7).
-define(OTP_VSN, "21.0.7").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 7).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_0_8).
-define(OTP_VSN, "21.0.8").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 8).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_0_9).
-define(OTP_VSN, "21.0.9").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 9).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_1_0).
-define(OTP_VSN, "21.1.0").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_1_1).
-define(OTP_VSN, "21.1.1").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_1_2).
-define(OTP_VSN, "21.1.2").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_1_3).
-define(OTP_VSN, "21.1.3").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_1_4).
-define(OTP_VSN, "21.1.4").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_2_0).
-define(OTP_VSN, "21.2.0").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_2_1).
-define(OTP_VSN, "21.2.1").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_2_2).
-define(OTP_VSN, "21.2.2").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_2_3).
-define(OTP_VSN, "21.2.3").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_2_4).
-define(OTP_VSN, "21.2.4").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_2_5).
-define(OTP_VSN, "21.2.5").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_2_6).
-define(OTP_VSN, "21.2.6").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 6).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_2_7).
-define(OTP_VSN, "21.2.7").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 7).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_3_0).
-define(OTP_VSN, "21.3.0").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_3_1).
-define(OTP_VSN, "21.3.1").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_3_2).
-define(OTP_VSN, "21.3.2").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_3_3).
-define(OTP_VSN, "21.3.3").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_3_4).
-define(OTP_VSN, "21.3.4").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_3_5).
-define(OTP_VSN, "21.3.5").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_3_6).
-define(OTP_VSN, "21.3.6").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 6).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_3_7).
-define(OTP_VSN, "21.3.7").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 7).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_21_3_8).
-define(OTP_VSN, "21.3.8").
-define(OTP_VSN_MAJOR, 21).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 8).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_0_0).
-define(OTP_VSN, "22.0.0").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_0_1).
-define(OTP_VSN, "22.0.1").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_0_2).
-define(OTP_VSN, "22.0.2").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_0_3).
-define(OTP_VSN, "22.0.3").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_0_4).
-define(OTP_VSN, "22.0.4").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_0_5).
-define(OTP_VSN, "22.0.5").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_0_6).
-define(OTP_VSN, "22.0.6").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 6).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_0_7).
-define(OTP_VSN, "22.0.7").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 7).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_1_0).
-define(OTP_VSN, "22.1.0").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_1_1).
-define(OTP_VSN, "22.1.1").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_1_2).
-define(OTP_VSN, "22.1.2").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_1_3).
-define(OTP_VSN, "22.1.3").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_1_4).
-define(OTP_VSN, "22.1.4").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_1_5).
-define(OTP_VSN, "22.1.5").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_1_6).
-define(OTP_VSN, "22.1.6").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 6).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_1_7).
-define(OTP_VSN, "22.1.7").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 7).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_1_8).
-define(OTP_VSN, "22.1.8").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 1).
-define(OTP_VSN_PATCH, 8).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_2_0).
-define(OTP_VSN, "22.2.0").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_2_1).
-define(OTP_VSN, "22.2.1").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_2_2).
-define(OTP_VSN, "22.2.2").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_2_3).
-define(OTP_VSN, "22.2.3").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 3).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_2_4).
-define(OTP_VSN, "22.2.4").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 4).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_2_5).
-define(OTP_VSN, "22.2.5").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 5).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_2_6).
-define(OTP_VSN, "22.2.6").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 6).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_2_7).
-define(OTP_VSN, "22.2.7").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 7).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_2_8).
-define(OTP_VSN, "22.2.8").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 2).
-define(OTP_VSN_PATCH, 8).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_3_0).
-define(OTP_VSN, "22.3.0").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_3_1).
-define(OTP_VSN, "22.3.1").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 1).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_22_3_2).
-define(OTP_VSN, "22.3.2").
-define(OTP_VSN_MAJOR, 22).
-define(OTP_VSN_MINOR, 3).
-define(OTP_VSN_PATCH, 2).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

-ifdef(_internal_otp_vsn_23_0_0).
-define(OTP_VSN, "23.0.0").
-define(OTP_VSN_MAJOR, 23).
-define(OTP_VSN_MINOR, 0).
-define(OTP_VSN_PATCH, 0).
-define(OTP_VSN_23_AND_ABOVE, true).
-define(OTP_VSN_22_AND_ABOVE, true).
-define(OTP_VSN_21_AND_ABOVE, true).
-define(OTP_VSN_20_AND_ABOVE, true).
-define(OTP_VSN_19_AND_ABOVE, true).
-define(OTP_VSN_18_AND_ABOVE, true).
-define(OTP_VSN_17_AND_ABOVE, true).
-define(OTP_VSN_16_AND_ABOVE, true).
-endif.

%% integer_to_list(Literal) resolves at compile time.
-define(OTP_VSN_MAJOR_STRING, integer_to_list(?OTP_VSN_MAJOR)).
-define(OTP_VSN_MINOR_STRING, integer_to_list(?OTP_VSN_MINOR)).
-define(OTP_VSN_PATCH_STRING, integer_to_list(?OTP_VSN_PATCH)).

-ifdef(OTP_VSN_17_AND_ABOVE).
-define(OTP_VSN_HAS_MAPS, true).
-endif.

-ifdef(OTP_VSN_21_AND_ABOVE).
-define(OTP_VSN_HAS_ST_MATCHING, true).
-define(OTP_VSN_IF_HAS_ST_MATCHING(Yes), Yes).
-define(OTP_VSN_IF_HAS_ST_MATCHING(Yes, No), Yes).
-define(OTP_VSN_STACKTRACE(Type, Error, ST), Type:Error:ST ->).
-else.
-define(OTP_VSN_IF_HAS_ST_MATCHING(Yes), ).
-define(OTP_VSN_IF_HAS_ST_MATCHING(Yes, No), No).
-define(OTP_VSN_STACKTRACE(Type, Error, ST), Type:Error -> ST = erlang:get_stacktrace(),).
-endif.

-endif.
